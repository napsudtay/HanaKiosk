...
async function btn_check_reservation_checkin_or_checkout() {  // อันนี้ให้ปุ่มมกดยืนยันข้อมูลเรียกใช้ เช็คให้ไปหน้าไหน
    ...
    if (page_reservation === 1) {
        ...
        await getToken();
        var from_date = tranformDate(reservation_checkin, "/");
        var to_date = tranformDate(reservation_checkout, "/");
        
        
        var prices = await get_DailyPrice(token, site_uuid, room_uuid_choose , from_date, to_date)
        console.log( 'prices=', prices );
        
        // var price_arr = [];
        // var price_addBed_arr = [];
        // var price_flex_arr = [];
        ...
        if(prices.length > 1){
            prices.pop(); //เอาราคาวันสุดท้ายออก เพราะไม่ได้พักคืนนั้น
        }
        prices.forEach((price_val) =>{
            // price_arr.push(price_val['pricing_config']['rate']);
            // price_addBed_arr.push(price_val['pricing_config']['extra_bed']);
            // price_flex_arr.push(price_val['pricing_config']['flex']);
            
            price += parseInt( price_val['pricing_config']['rate'] );
            price_addBed += parseInt( price_val['pricing_config']['extra_bed'] );
            price_flex += parseInt( price_val['pricing_config']['flex'] );
        });
        // console.log( price );
        // console.log( price_addBed );
        // console.log( price_flex );
        
        var total_price = (price * quantity_choose) + (price_addBed * addBed_choose) ;
        ...
        if (phoneNumberReservation.length !== 10) { // หมายเลขโทรศัพท์ไม่ถูกต้องหรือไม่ถึง 10 ตัว
            PopupWarningWindows();
        ...
        document.getElementById("modal-reservation-successful-sendroomnumber").innerText = generateRandomNumber();
